...
class FedNova(FedAvg):
    ...
    def aggregate_fit(self, server_round: int, results: List[Tuple[ClientProxy, FitRes]], failures: List[Tuple[int, Exception]]) -> Tuple[Parameters, Dict[str, float]]:
        ...
        for client, fit_res in results:
            ...
            # Ensure the normalized values are within a reasonable range
            for i, param in enumerate(normalized_updates):
                logging.info(f"Normalized parameter {i}: mean={np.mean(param)}, std={np.std(param)}")
        ...
        # Log aggregated parameters before conversion
        for i, param in enumerate(aggregated_parameters_ndarrays):
            logging.info(f"Aggregated parameter {i}: mean={np.mean(param)}, std={np.std(param)}")
        ...
        sample_input = torch.randn(1, input_dim).to(DEVICE)  # Create a sample input tensor
        model.eval()
        with torch.no_grad():
            sample_output = model(sample_input)
        logging.info(f"Sample model output: {sample_output}")

        # Check if sample output is within range [0, 1]
        if not torch.all(sample_output >= 0) or not torch.all(sample_output <= 1):
            logging.warning("Sample model output contains values outside the range [0, 1]")
