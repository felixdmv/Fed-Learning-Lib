...
# def parameters_to_state_dict(aggregated_parameters: Dict[str, torch.Tensor], model: torch.nn.Module) -> Dict[str, torch.Tensor]:
#     """Convert aggregated parameters to state_dict format."""
#     state_dict = model.state_dict()
#     for name, param in state_dict.items():
#         if name in aggregated_parameters:
#             state_dict[name] = aggregated_parameters[name]
#     return state_dict

# class FedNova(FedAvg):

#     def __init__(self, save_model_directory: str, *args, **kwargs):
#         super().__init__(*args, **kwargs)
#         self.save_model_directory = save_model_directory
#         self.previous_model_parameters = None

#     def aggregate_fit(self, rnd: int, results: List[Tuple[ClientProxy, FitRes]], failures: List[Union[Tuple[ClientProxy, FitRes], BaseException]]) -> Parameters:
#         """Aggregate fit results from clients using FedNova strategy."""
#         if not results:
#             return None
...
#         logging.info(f"Model state_dict keys: {list(model_state_dict.keys())}")

#         aggregated_gradients = {name: torch.zeros_like(param) for name, param in model_state_dict.items()}
...
#             client_model_params = {name: torch.tensor(param) for name, param in zip(model_state_dict.keys(), client_parameters)}

#             if self.previous_model_parameters is not None:
#                 for name, client_param in client_model_params.items():
#                     previous_param = self.previous_model_parameters[name]
#                     normalized_gradient = (client_param - previous_param) / num_examples_client
#                     aggregated_gradients[name] += normalized_gradient * num_examples_client

#         for name in aggregated_gradients:
#             aggregated_gradients[name] /= total_examples

#         state_dict = parameters_to_state_dict(aggregated_gradients, model)
...
#         logging.info(f'Saved global model for round {rnd} at {os.path.join(round_dir, "aggregated_model.pth")}')

#         self.previous_model_parameters = {name: param.clone().detach() for name, param in model.state_dict().items()}

#         aggregated_parameters = ndarrays_to_parameters([param.cpu().numpy() for param in model.state_dict().values()])
